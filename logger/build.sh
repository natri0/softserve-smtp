...
case $TARGET in
    lib)
        CMAKE_ARGS="-DCMAKE_BUILD_TYPE=${BUILD_TYPE} -DENABLE_TESTS=OFF -DENABLE_BENCHMARKS=OFF"
        BUILD_TARGET="logger"
        ;;
    example)
        CMAKE_ARGS="-DCMAKE_BUILD_TYPE=${BUILD_TYPE} -DENABLE_TESTS=OFF -DENABLE_BENCHMARKS=OFF"
    ...
    stressTest)
        ...
        ;;
    unitTests)
        CMAKE_ARGS="-DCMAKE_BUILD_TYPE=${BUILD_TYPE} -DENABLE_TESTS=ON -DENABLE_BENCHMARKS=OFF"
        BUILD_TARGET="logger_tests"
        ;;
    benchmarks)
        CMAKE_ARGS="-DCMAKE_BUILD_TYPE=${BUILD_TYPE} -DENABLE_TESTS=OFF -DENABLE_BENCHMARKS=ON"
        BUILD_TARGET="logger_benchmark"
        ;;
    all)
        CMAKE_ARGS="-DCMAKE_BUILD_TYPE=${BUILD_TYPE} -DENABLE_TESTS=ON -DENABLE_BENCHMARKS=ON"
        BUILD_TARGET="all"
        ;;
esac
